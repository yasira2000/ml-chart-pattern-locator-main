...
const API_BASE_URL =
  ...
  "https://pattern-detection-django.onrender.com/api/v1";
...
const ChartContainer: React.FC = () => {
  ...
  const loadAllData = async (selectedSymbol: string) => {
    ...
    try {
      ...
      const response = await axios.get<OHLCData[]>(
        `${API_BASE_URL}/ohlc-data/`,
        {
          params: {
      ...
      if (candlestickSeriesRef.current) {
        const chartData: CandlestickData[] = sortedData.map(
          (item: OHLCData) => ({
            time: item.Date,
  ...
  const detectPatternsInRange = async (startDate: Date, endDate: Date) => {
    ...
    try {
      ...
      if (candlestickSeriesRef.current) {
        const markers: ChartMarker[] = response.data
          .flatMap((pattern: Pattern, index: number) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <TimelineRangeSelector
        visibleStartDate={
          allData.length > 0 ? new Date(allData[0].Date) : new Date()
        }
        visibleEndDate={
          allData.length > 0
